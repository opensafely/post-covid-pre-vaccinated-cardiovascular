...
## 
## Content:  to create date for Venn diagrams for each outcome outlining overlap in 
...
outcomes <- (active_analyses %>% filter(active == "TRUE") %>% select(outcome_variable))[[1]]
...
input_stage1 <- readr::read_rds(paste0("output/input_stage1.rds"))
...
# Save summary file ----------------------------------------------------------
write.csv(df, file = paste0("output/review/venn-diagrams/venn_diagram_number_check_pre_vaccination_cvd_NOT_REDACTED.csv"), row.names = F)
...
#change NAs to 0
df[is.na(df)] <- 0
write.csv(df, file = paste0("output/review/venn-diagrams/venn_diagram_number_check_pre_vaccination_cvd.csv"), row.names = F)
