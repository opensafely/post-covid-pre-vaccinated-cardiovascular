...
for (i in numeric_var_names){
  ...
  check_range[nrow(check_range),2] <- min(na.omit(input %>% dplyr::select(all_of(i))))
  check_range[nrow(check_range),3] <- max(na.omit(input %>% dplyr::select(all_of(i))))
}
...
input <- input %>% mutate(exp_date_covid19_confirmed = replace(exp_date_covid19_confirmed, which(exp_date_covid19_confirmed>cohort_end_date | exp_date_covid19_confirmed<cohort_start_date), NA))
...
table1 <- input %>% 
  dplyr::select(c(all_of(categorical_cov),all_of(numerical_cov)))
...
# Save table 1
write.csv(table1_suppressed, file = file.path("output/review/descriptives", paste0("Table1_pre_vaccination_cvd.csv")) , row.names=F)
