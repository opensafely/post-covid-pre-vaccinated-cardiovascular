/*----------------------------------------------------------------------------
Do file name: 			cox_model for extended follow up day zero
...
stsplit days, after(exposure_date) at(0 1 28 197 365 714)

* Calculate study follow up

replace days = 714 if days==-1
...
gen days197_365 = 0
replace days197_365 = 1 if days==197
tab days197_365

gen days365_714 = 0
replace days365_714 = 1 if days==365
tab days365_714
...
stcox days0_1 days1_28 days28_197 days197_365 days365_714 i.sex age_spline1 age_spline2, strata(region) vce(r)
est store min, title(Age_Sex)
stcox days0_1 days1_28 days28_197 days197_365 days365_714 i.sex age_spline1 age_spline2 i.cov_cat_ethnicity i.cov_cat_deprivation i.cov_cat_smoking_status cov_num_consulation_rate cov_bin_*, strata(region) vce(r)
est store max, title(Maximal)

estout * using "output/`cpf'_cox_model_extended_follow_up_day_zero.txt", cells("b se t ci_l ci_u p") stats(risk N_fail N_sub N N_clust) replace 

* Calculate median follow-up

keep if outcome_status==1
keep patient_id days0_1 days1_28 days28_197 days197_365 days365_714 follow_up
...
export delimited using "output/`cpf'_stata_median_fup_extended_follow_up_day_zero", replace
