...
input <- input[,c("patient_id","death_date","index_date","sub_cat_covid19_hospital","vax_date_eligible","cohort_end_date", 
                  active_analyses$outcome_variable[!grepl("extended_follow_up",active_analyses$outcome_variable)],
...
# Create extended follow up outcome event variables
for(extended_follow_up_event in active_analyses$outcome_variable[grepl("extended_follow_up",active_analyses$outcome_variable)]){
  event <- sub("_extended_follow_up", "", extended_follow_up_event)
  input[,extended_follow_up_event] <- input[,event] 
}

event <- "out_date_ami_extended_follow_up"

for(event in active_analyses$outcome_variable[!grepl("extended_follow_up",active_analyses$outcome_variable)]){
