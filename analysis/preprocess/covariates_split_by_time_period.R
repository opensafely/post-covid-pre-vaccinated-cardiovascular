...
#These are the study start and end dates for the pre-vaccination period
...
time_periods_normal <- c(7, 14, 28, 56, 84, 196,365,535) 
time_periods_reduced <- c(28,535)

time_periods <- time_periods_normal
event_by_covariate_level <- function(time_periods,save_name){
  
  # define analyses of interests
  active_analyses <- read_rds("lib/active_analyses.rds")
  active_analyses <- active_analyses %>%dplyr::filter(active == "TRUE")
  
  analyses_of_interest <- as.data.frame(matrix(ncol = 4,nrow = 0))
  ...
  write.csv(results, file=paste0(output_dir,"/event_counts_by_covariate_level_",save_name,"_time_periods.csv"), row.names = F)
...
select_covariates_for_cox <- function(results, save_name,time_periods_names, active_analyses,cohort_name){
      ...
      tmp <- tmp %>%
        ...
        dplyr::mutate(keep_covariate = case_when(
          any(unexposed_event_counts == 0 | days0_7_event_counts == 0 | days7_14_event_counts == 0 |
                days14_28_event_counts == 0 | days28_56_event_counts ==0 | days56_84_event_counts ==0 | days84_196_event_counts ==0 |
                days196_365_event_counts == 0 | days365_535_event_counts ==0 ) ~ "remove_covariate",
          TRUE ~ "keep_covariate"))
    }else if(save_name == "reduced"){
      tmp <- tmp %>%
        group_by(event, covariate, subgroup) %>%
        dplyr::mutate(keep_covariate = case_when(
          any(unexposed_event_counts == 0 | days0_28_event_counts == 0 | days28_535_event_counts == 0) ~ "remove_covariate",
          TRUE ~ "keep_covariate"))
    }
  ...
  write.csv(covariates_to_adjust_for, file=paste0(output_dir,"/non_zero_selected_covariates_",save_name,"_time_periods.csv"), row.names = F)
  
}

# Run function for each time period
event_by_covariate_level(time_periods_normal,"normal")
event_by_covariate_level(time_periods_reduced,"reduced")
